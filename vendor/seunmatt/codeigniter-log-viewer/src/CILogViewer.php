...
class CILogViewer extends \App\Controllers\BaseController {
    ...
    const LOG_LINE_START_PATTERN = "/((INFO)|(ERROR)|(DEBUG)|(ALL)|(CRITICAL)|(ALERT)|(NOTICE))[\s\- \d:]+(-->)/";
    const LOG_DATE_PATTERN       = "/(\d{4,}-[\d:\-]{2,})\s([\d:]{2,})/";
    const LOG_LEVEL_PATTERN      = "/^((ERROR)|(INFO)|(DEBUG)|(CRITICAL)|(ALERT)|(NOTICE))/";
    ...
    private $LOG_VIEW_FILE_NAME   = "VIEW__logs.php";
    private $LOG_VIEW_FILE_PATH   = "";

    //this is the name of the view file passed to CI load->view()
    const CI_LOG_VIEW_FILE_PATH = APPPATH . "Views/system/";
    const MAX_LOG_SIZE          = 52428800; //50MB
    const MAX_STRING_LENGTH     = 1000; //300 chars
    ...
    private function init() {
        ...
        $this -> logFolderPath  = "writable/logs";
        $this -> logFilePattern = "log-*";
        ...
        $this -> LOG_VIEW_FILE_FOLDER = APPPATH . "Views/system";
    ...
    public function showLogs($request) {
        ...
        if (!is_null($request -> getVar("del"))) {
            $this -> deleteFiles(base64_decode($request -> getVar("del")));
            //return redirect() -> route('LogViewer');
            return redirect() -> back();
            //header("Location:".base_url("systems/LogViewer"));
        }
        ...
        if (!is_null($request -> getVar(self::API_QUERY_PARAM))) {
            return $this -> processAPIRequests($request -> getVar(self::API_QUERY_PARAM));
        }

        //it will either get the value of f or return null
        $fileName = $request -> getVar("f");
        ...
        echo view($_ENV['app.theme'] . '/layouts/header/VIEW__head', $data);
        echo view('system/VIEW__logs', $data);

        // return $this -> render_template('system/VIEW__logs', $data);
    ...
    private function processAPIRequests($command) {
        ...
        if ($command === self::API_CMD_LIST) {
            ...
            $file                  = $this -> request -> getVar(self::API_FILE_QUERY_PARAM);
            $response["log_files"] = self::getFilesBase64Encoded();
            ...
            if (is_null($file) || empty($file)) {
                ...
                $singleLine         = $this -> request -> getVar(self::API_LOG_STYLE_QUERY_PARAM);
