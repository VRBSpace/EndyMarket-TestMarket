...
if (!function_exists('HELPER__build_breadcrumbs')) {

    /**
     * Генерира breadcrumb масив и HTML, базирани на продукт и категории
     * @param object $product Обект на продукта (DB row)
     * @param array $categories Дърво с категории
     * @return string HTML <nav> с <ol class="breadcrumb">
     */
    function HELPER__build_breadcrumbs(object $product, array $categories): string {
...
if (!function_exists('render_breadcrumb')) {
    ...
    function render_breadcrumb(array $items, string $class = 'breadcrumb'): string {
        if (empty($items))
            return '';
