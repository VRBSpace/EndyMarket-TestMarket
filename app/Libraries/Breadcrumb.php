...
class Breadcrumb {
    ...
    public function __construct() {
        ...
        $this -> tags['liopen']       = "<li class=\"breadcrumb-item text-black\">";
        $this -> tags['liopenItalic'] = "<li class=\"breadcrumb-item font-italic\">";
    ...
    public function add($crumb, $href = '') {
        ...
        // if (!$crumb or!$href)
        //return; // if the title or Href not set return 
    ...
    public function render() {
        ...
        foreach ($this -> breadcrumbs as $index => $breadcrumb) {
            $href = trim((string) ($breadcrumb['href'] ?? ''));
            ...
            if ($index == $count) {
                ...
                $output .= $this -> tags['liopenItalic'];

                if ($index === 0 || $href !== '') {
                    if ($href === '') {
                        $url = base_url();
                    } elseif (preg_match('~^(https?:)?//~', $href) || strpos($href, '/') === 0) {
                        $url = $href;
                    } else {
                        $url = base_url($href);
                    }

                    $output .= '<a href="' . $url . '">' . $breadcrumb['crumb'] . '</a>';
                } else {
                    $output .= $breadcrumb['crumb'];
                }
